...
impl NodeStore for SqliteVpnStore {
    async fn add_node(&mut self, node: &Node) -> VpnResult<()> {
        ...
        self.conn
            .execute_sql(sql_query(sql).bind(&node.id.to_base58()))
            .await
            .map_err(into_vpn_err!(VpnErrorCode::IoError))?;
        Ok(())
    }
    ...
    async fn remove_node(&mut self, id: &NodeId) -> VpnResult<()> {
        ...
        self.conn
            .execute_sql(sql_query(sql).bind(&id.to_base58()))
            .await
            .map_err(into_vpn_err!(VpnErrorCode::IoError))?;
        Ok(())
    }
    ...
    async fn get_node(&mut self, id: &NodeId) -> VpnResult<Option<Node>> {
        ...
        match self
            .conn
            .query_one(sql_query(sql).bind(&id.to_base58()))
            .await
        {
            Ok(row) => {
...
impl NetworkStore for SqliteVpnStore {
            ...
            .await
        {
            Ok(row) => {
                ...
                Ok(Some(JoinedNode {
                    ...
                    node_id: NodeId::from_base58(&node_id)
                        .map_err(into_vpn_err!(VpnErrorCode::IoError))?,
                    allow_join,
                    name,
                    comment,
    ...
    async fn get_joined_network_group(&mut self, node_id: &NodeId) -> VpnResult<Vec<JoinedNode>> {
        ...
        let rows = self
            .conn
            .query_all(sql_query(sql).bind(&node_id.to_base58()))
            .await
            .map_err(into_vpn_err!(VpnErrorCode::IoError))?;
        let mut nodes = Vec::new();
        for row in rows {
