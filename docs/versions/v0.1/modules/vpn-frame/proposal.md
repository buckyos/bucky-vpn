...
approved_content_sha256: 3220b14429141d85a49b742033f91843963eee9a34158dda76e6a6765b401420
---
...
|------------------|---------|----------|-----------------|----------------------------|
| contract/protocol | yes | `NodeNetwork.pn_server` and traffic-report payload carry serialized PN server data. | schema-check; admission-check; cargo check/test through vpn-frame harness entry | none |
| data/schema | yes | SQLite network and proxy-node PN server storage changes from endpoint string to structured id/ip/port fields. | testing must record schema and compatibility boundaries | owner: vpn-frame maintainers; risk: existing endpoint-string rows are unsupported by requirement; acceptance impact: acceptance must verify no legacy endpoint compatibility claim remains |
