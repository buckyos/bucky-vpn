...
#[async_trait]
pub trait VpnClientFactory<M: CmdTunnelMeta,
    CS: CmdSend<M>,
    G: SendGuard<M, CS>,
    ...
    L: VpnTunnelListener<R, S>>: Send + Sync + 'static {
...
impl<M: CmdTunnelMeta,
    ...
    F: VpnTunnelFactory<R, S>,
    L: VpnTunnelListener<R, S>,
    CF: VpnClientFactory<M, CS, G, T, R, S, F, L>> VpnClientManager<M, CS, G, T, R, S, F, L, CF> {
    ...
    pub async fn get_client(&self, key: &str) -> VpnResult<Arc<VpnClient<M, CS, G, T, R, S, F, L>>> {
        {
