...
#[async_trait]
pub trait VpnClientFactory<T: CmdClient<u16, u8>, R: VpnTunnelRecv, S: VpnTunnelSend, F: VpnTunnelFactory<R, S>, L: VpnTunnelListener<R, S>>: Send + Sync + 'static {
    async fn create_client(&self, key: &str) -> VpnResult<Arc<VpnClient<T, R, S, F, L>>>;
}
...
pub struct VpnClientManager<
    ...
    CF: VpnClientFactory<T, R, S, F, L>> {
    clients: Mutex<HashMap<String, Arc<VpnClient<T, R, S, F, L>>>>,
    factory: Arc<CF>,
}
...
impl<T: CmdClient<u16, u8>,
    ...
    F: VpnTunnelFactory<R, S>,
    L: VpnTunnelListener<R, S>,
    CF: VpnClientFactory<T, R, S, F, L>> VpnClientManager<T, R, S, F, L, CF> {
    ...
    pub async fn get_client(&self, key: &str) -> VpnResult<Arc<VpnClient<T, R, S, F, L>>> {
        {
