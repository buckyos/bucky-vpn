...
use sfo_http::openapi::{utoipa, OpenApiServer, OpenapiNoValueResult};
...
use crate::p2p_vpn::{vpn_client_manager, JoinRecord};
...
impl Api {
    pub fn register_api<
        Req: Request,
        Resp: Response,
        S: HttpServer<Req, Resp> + OpenApiServer,>(server: &mut S, setting: Arc<Setting>) {
        def_openapi! {
                [login]
                #[utoipa::path(
                    post,
