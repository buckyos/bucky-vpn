...
impl Setting {
    pub async fn load(config_file: &Path) -> VpnResult<Setting> {
        let config = if config_file.exists() {
            ...
            toml::from_str(content.as_str()).map_err(into_vpn_err!(
                VpnErrorCode::Failed,
    ...
    pub fn set<T: serde::ser::Serialize>(&self, key: &str, value: T) -> VpnResult<()> {
        ...
        let value = serde_json::to_value(value).map_err(into_vpn_err!(
            VpnErrorCode::Failed,
            "serialize config {} failed",
            key
        ))?;
    ...
    pub async fn save(&self) -> VpnResult<()> {
        let toml_string = {
            let config = self.config.lock().unwrap();
            toml::to_string(config.deref())
                .map_err(into_vpn_err!(VpnErrorCode::Failed, "toml to string failed"))?
        };
        tokio::fs::write(&self.config_file, toml_string)
            .await
            .map_err(into_vpn_err!(
                VpnErrorCode::Failed,
