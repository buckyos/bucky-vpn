...
- After each child task completes, the pipeline updates the task-local `pipeline/state.json` task status to `confirmed` or `complete` before continuing to dependent tasks
- Implementation completion is recorded in task-local `pipeline/state.json` and implementation evidence, and final acceptance is recorded in task-local `pipeline/state.json` and the acceptance report
...
- Each child task MUST keep writes inside its named stage artifact group unless the user explicitly requested cross-stage synchronization for that task. Narrow workflow-required companion writes are allowed: design/implementation/testing child tasks may update `pipeline/plan.md`; testing child tasks may update `harness/scripts/test-run.py` and write `test-results/test-runs/*.json` run evidence
- Each single-stage child task MUST record its changed paths, then run `stage-scope-check.py --stage <stage> --changed-paths-file docs/versions/<version>/evidence/stage-scope/<task-id>.paths` before completion and fail on out-of-stage task paths
